		   ...
		   value w, value h) /* ML */
{
  ...
  pib.offset[1] = 1;
  pib.offset[2] = 2;
  /* FIXME: Always assumes that the compRule is TK_PHOTO_COMPOSITE_OVERLAY. */
  Tk_PhotoPutBlock(cltclinterp, ph,&pib,Int_val(x),Int_val(y),Int_val(w),Int_val(h), TK_PHOTO_COMPOSITE_OVERLAY); 
}
